...
namespace kepler {
	...
	void DX11Shader::Compile(ID3DBlob* pOutBlob, const std::string& filepath, const std::string& entryPointName)
	{
		...
		HRESULT hr = D3DCompileFromFile(wFilePath.c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, entryPointName.c_str(), target.c_str(), shaderFlags, 0, &pOutBlob, &pErrorBlob);
		if (FAILED(hr))
		{
