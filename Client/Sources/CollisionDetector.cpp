...
bool CollisionDetector::Detection()
{
	for (int i = 0; i < s_pColliders.size() - 1; i++)
	{
		...
		{
			// Box to Box - AABB
			if (s_pColliders[i]->GetColliderType() == eColliderType::Box && s_pColliders[i]->GetColliderType() == eColliderType::Box)
			{
			...
			else if (s_pColliders[i]->GetColliderType() == eColliderType::Circle && s_pColliders[i]->GetColliderType() == eColliderType::Circle)
			{
			...
			else
			{
				...
				if (!isCollided)
				{
					...
					if ((segPoint - circleObj->GetPosition()).Length() < circleObj->GetSize().x)
					{
						isCollided = true;
					}
				}
				...
				if (!isCollided)
				{
					...
					if ((segPoint - circleObj->GetPosition()).Length() < circleObj->GetSize().y)
					{
						isCollided = true;
					}
				}
